...
class JaiAgentService : AccessibilityService() {
    ...
    override fun onServiceConnected() {
        ...
        instance = this
        Log.d("JaiAgent", "Accessibility Agent Connected & Ready")
    }

    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        if (event == null) return

        // 1. Keep track of the active app
        ...
        // 2. Automated Text Typing into ANY focused field (WhatsApp, Claude, Gemini, Notes)
        ...
        // 3. Automated WhatsApp Dispatch & Send Click
    ...
    companion object {
        ...
        fun executeCommand(context: Context, responseText: String): Boolean {
            ...
            return when {
                // 1. OPEN APPS BY NAME (WhatsApp, Claude, Gemini, YouTube, Camera, Gmail)
                ...
                // 2. TYPE TEXT INTO ACTIVE INPUT FIELD
                trimmed.startsWith("ACTION:TYPE:") -> {
                    val textToType = trimmed.removePrefix("ACTION:TYPE:").trim()
                    pendingTextToType = textToType
                ...
                // 5. SCHEDULE REMINDER / ALARM / WORK EVENT
                ...
                // 6. SEND EMAIL / GMAIL DRAFT
                trimmed.startsWith("ACTION:EMAIL:") -> {
                    val body = trimmed.removePrefix("ACTION:EMAIL:").trim()
                    val intent = Intent(Intent.ACTION_SENDTO).apply {
                        data = Uri.parse("mailto:")
                        putExtra(Intent.EXTRA_SUBJECT, "Scheduled Message from JAI")
