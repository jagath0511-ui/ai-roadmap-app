...
class FloatingOverlayService : Service(), TextToSpeech.OnInitListener {
    ...
    private var clipboardWatcherManager: ClipboardWatcherManager? = null
    ...
    override fun onCreate() {
        ...
        // 1. Initialize Hardware Gesture Awakener
        ...
        // 2. Initialize Proactive Clipboard Watcher
        clipboardWatcherManager = ClipboardWatcherManager(this) { clipItem ->
            handleClipboardEvent(clipItem)
        }
        clipboardWatcherManager?.startListening()

        // 3. Hook into JaiAgentService result listener
    ...
    private fun handleClipboardEvent(clipItem: ClipItem) {
        when (clipItem.type) {
            ClipType.URL_LINK -> {
                deckManager.updateStatus("Copied Link: ${clipItem.text.take(25)}...")
                speakOutResponse("Link copied. Say browse or brain to analyze.")
            }
            ClipType.PHONE_NUMBER -> {
                deckManager.updateStatus("Copied Phone: ${clipItem.text}")
                speakOutResponse("Phone number copied. Say call to dial.")
            }
            ClipType.GENERAL_TEXT -> {
                deckManager.updateStatus("Copied: ${clipItem.text.take(20)}...")
            }
        }
    }
    ...
    private fun startForegroundServiceSafe() {
        try {
            ...
            val notification: Notification = NotificationCompat.Builder(this, channelId)
                ...
                .setContentText("Listening for gestures & clipboard events")
    ...
    override fun onDestroy() {
        try {
            clipboardWatcherManager?.stopListening()
