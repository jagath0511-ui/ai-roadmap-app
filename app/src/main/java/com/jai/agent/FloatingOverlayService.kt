...
class FloatingOverlayService : Service(), TextToSpeech.OnInitListener {
    ...
    private fun startForegroundServiceNotification() {
        ...
        val notification: Notification = NotificationCompat.Builder(this, channelId)
            ...
            .setContentText("Hands-free OS Assistant & Live HUD running")
    ...
    private fun buildFloatingDeckUi() {
        ...
        val btnSend = Button(this).apply {
            ...
            setOnClickListener {
                ...
                if (query.isNotEmpty()) {
                    promptEditText.setText("") // Clean auto-clear
    ...
    private fun toggleLiveCamera(isFront: Boolean) {
        ...
        if (isCameraActive && activeLensFacing == targetFacing) {
            // Same button tapped again -> Close camera
            closeCamera()
            cameraContainer.visibility = View.GONE
            updateStatus("Camera View Closed")
        } else {
            // Switching lens or opening for the first time
            if (isCameraActive) {
                closeCamera()
            }
    ...
    // Thread-safe UI update dispatcher (Prevents background thread crashes)
    ...
    override fun onDestroy() {
        serviceJob.cancel() // Cancel all active coroutines safely
        closeCamera()
