...
class JaiAgentService : AccessibilityService() {
    ...
    companion object {
        ...
        fun openApp(context: Context, appNameRaw: String): ActionResult {
            ...
            // 1. Direct package overrides for high-priority apps
            val directPackages = when {
                cleanQuery.contains("youtube") -> listOf("com.google.android.youtube")
                cleanQuery.contains("whatsapp") -> listOf("com.whatsapp", "com.whatsapp.w4b")
                cleanQuery.contains("gemini") -> listOf("com.google.android.apps.bard", "com.google.android.googlequicksearchbox")
                cleanQuery.contains("paytm") -> listOf("net.one97.paytm")
                cleanQuery.contains("chrome") -> listOf("com.android.chrome")
                cleanQuery.contains("instagram") -> listOf("com.instagram.android")
                cleanQuery.contains("camera") -> listOf("com.android.camera", "com.google.android.GoogleCamera")
                cleanQuery.contains("settings") -> listOf("com.android.settings")
                else -> emptyList()
            }
            ...
            // 2. Full device scan across all installed launcher packages
            try {
                ...
                FailureLogger.log(context, "openApp", "Scan error: ${e.localizedMessage}")
            }
